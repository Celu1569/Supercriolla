    ...
    if ((finalTitle || finalArtist) && !isStationName(finalTitle) && !isStationName(finalArtist)) {
        try {
          ...
          if (searchTerm.length > 2) {
            ...
            const itunesResponse = await axios.get(itunesUrl, { timeout: 3500, signal: controller.signal });
            if (itunesResponse.data?.results?.[0]?.artworkUrl100) {
              ...
              if (titleOnlyTerm.length > 2) {
                ...
                const fallbackRes = await axios.get(fallbackUrl, { timeout: 2500, signal: controller.signal });
